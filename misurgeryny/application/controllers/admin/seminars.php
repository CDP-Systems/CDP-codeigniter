...
class Seminars extends MY_Controller {    
    ...
    public function __construct()
    {
        ...
        $this->load->model('default/m_seminar_attendees', 'seminar_attendees');
    ...
    function index()
    {
        $this->view_data['content'] = 'admin/seminars/list';
    ...
    function add()
    {
        $this->view_data['content'] = 'admin/seminars/edit';

        $this->_prep_form_values('validation_seminar_form');

        if ($this->input->post('submit'))
        {
            if ($this->_save_seminar())
            {
                $this->session->set_flashdata('message', 'Seminar saved!');
    ...
    function edit()
    {
        ...
        $this->_prep_form_values('validation_seminar_form', $seminar);

        if ($this->input->post('submit'))
        {
            if ($this->_save('validation_seminar_form', $this->seminars))
            {
                $this->session->set_flashdata('message', 'Seminar updated!');
    ...
    function logs()
    {
        $this->view_data['content'] = 'admin/seminars/log';
    ...
    {
        $this->view_data['content'] = 'admin/seminars/view_attendee';
    ...
    function delete_seminar($id)
    {
        ...
        {
            $this->session->set_flashdata('message', 'Seminar/s successfully deleted.');
        }
        else
        {
            $this->session->set_flashdata('message', 'Could not delete the seminar. Please contact the administrator.');
        }

        redirect('admin/seminars/index');
    }
    ...
    function email_settings()
    {
        ...
        $seminars_email_recipient = $this->m_settings->get('seminars_email_recipient');
        // Email content.
        $seminars_patient_confirmation = $this->m_settings->get('seminars_email_patient');
        $seminars_admin_notification = $this->m_settings->get('seminars_email_admin');
        // Email Subject.
        $seminars_patient_confirmation_subject = $this->m_settings->get('seminars_email_patient_subject');
        $seminars_admin_notification_subject = $this->m_settings->get('seminars_email_admin_subject');
        
        $this->_form_data['seminars_email_recipient'] = set_value('seminars_email_recipient', $seminars_email_recipient->setting_value);

        $this->_form_data['seminars_email_patient'] = set_value('seminars_email_patient', $seminars_patient_confirmation->setting_value);
        $this->_form_data['seminars_email_patient_subject'] = set_value('seminars_email_patient_subject', $seminars_patient_confirmation_subject->setting_value);
       
        $this->_form_data['seminars_email_admin'] = set_value('seminars_email_admin', $seminars_admin_notification->setting_value);
        $this->_form_data['seminars_email_admin_subject'] = set_value('seminars_email_admin_subject', $seminars_admin_notification_subject->setting_value);
        ...
        if ($this->input->post('submit') == 'Save')
        {
            ...
            $this->require_validation($this->config->item('seminars_email_settings'));

            if ($this->form_validation->run())
            {
                if ($this->m_settings->save_settings($this->_form_data))
                {
                    $this->session->set_flashdata('message', '<div class="green bold">Settings saved!</div>');

                    redirect ('admin/seminars/email_settings');
                }
            }
        }

        $this->view_data['content'] = 'admin/seminars/email_settings';
        //parse template
        $this->parser->parse('admin/template', $this->view_data);
    }
    ...
    private function _save_seminar()
    {
        ...
        $this->load->config('validations');
        $config = $this->config->item('validation_seminar_form');
	...
	function action($record){ 
		...
		redirect('admin/seminars/'.$redirect.'/'.$uri_4);
	}
	
	function export(){
		$this->load->helper('csv');
		$website = $this->M_website->getName();
		$sql = "SELECT l.*, s.title AS 'seminar title' FROM ci_seminar_attendees l LEFT JOIN ci_seminars s
			ON l.seminar_id = s.seminar_id";
		$query = $this->db->query($sql);
		query_to_csv($query, TRUE, $website.'-Seminars-Log.csv');
	}
